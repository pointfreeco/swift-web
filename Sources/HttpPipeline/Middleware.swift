...
public func writeHeader<A>(_ header: HTTPField) -> Middleware<HeadersOpen, HeadersOpen, A, A> {
  return pure <<< (\.response.headers %~ { $0 + [header] })
}

public func writeHeaders<A>(_ headers: HTTPFields) -> Middleware<HeadersOpen, HeadersOpen, A, A> {
  return pure <<< (\.response.headers %~ { $0 + headers })
}

public func writeHeaders<A>(_ headers: [HTTPField]) -> Middleware<HeadersOpen, HeadersOpen, A, A> {
  return pure <<< (\.response.headers %~ { $0 + headers })
}

public func writeHeader<A>(_ name: HTTPField.Name, _ value: String) -> Middleware<HeadersOpen, HeadersOpen, A, A> {
  return writeHeader(.init(name: name, value: value))
}
